...
你的核心任务是根据公司的《订阅与服务层级协议》为客户计算订阅费用。
你需要写一个 Prompt，把用户的输入转换成我们规定的 JSON 格式。
📚 知识库素材 (Knowledge Base)
《SyncFlow 订阅与服务层级协议 (v4.0)》

[T.1] 核心订阅套餐：客户必须选择一个套餐。**创业版**（支持最多 10 名用户）每月 50 美元；**企业版**（支持最多 50 名用户）每月 180 美元。
[T.2] 年度计费折扣：若选择按年一次性付费，客户可享受“付10个月送2个月”的优惠，即按月费的10倍支付全年费用。此折扣**仅适用**于[T.1]的核心订阅套餐费用。
[T.3] 高级功能模块：**高级分析与报告**，每月额外收费 30 美元；**企业级安全与合规包**，每月额外收费 50 美元。此模块费用不参与任何折扣。
[T.4] 强制性入驻培训：所有新开通的账户，均需支付一笔 300 美元的一次性**强制性入驻培训费**，以确保团队正确使用本软件。如果客户选择**年度计费**，此项费用将被**豁免**。
[T.5] 用户超额费用：若账户内的实际用户数超出所选套餐的限制，每位超出的用户将按每月 12 美元的价格收取超额费用。此费用按月计算，不享受任何折扣。

### 输入描述：

一段包含客户选择的套餐、计费周期、团队实际用户数、所需的高级模块以及是否为新账户的自然语言描述。

### 输出描述：

```json
{
  "initial_payment": int, //首次需支付的总金额。年付的话则为全年费用。
  "recurring_monthly_bill": int, //后续月份的常规月度账单。年付的话为0
  "variable_user_overage": int, //当月产生的用户超额费用
  "applied_rules": ["\[T.X\]"] //触发的规则编号，按编号升序排列
}
```
...
你的核心任务是根据公司的《每周订阅配置方案》为客户计算每周的订阅费用。
请你写prompt完成这个任务。
📚 知识库素材 (Knowledge Base)
《Gourmet Box 每周订阅配置方案 (v2.0)》

[M.1] 核心餐食计划：客户需选择一个基础计划。**双人计划**（提供2人份食材）每周 80 美元；**家庭计划**（提供4人份食材）每周 140 美元。
[M.2] 食谱偏好附加费：客户可以选择特殊的食谱系列，这会产生附加费用。**素食主义**系列不产生额外费用；**高蛋白/生酮**系列，需在核心餐食计划费用的基础上加收 15% 的附加费。
[M.3] 每周精选加购：客户可选择每周变化的精选商品。**手工奶酪拼盘**，每周额外收费 25 美元；**时令水果箱**，每周额外收费 20 美元。此费用不参与任何折扣。
[M.4] 配送区域费用：配送费根据客户所在区域决定。**A区（市区）** 每周 10 美元；**B区（郊区）** 每周 18 美元。
[M.5] 忠诚会员奖励：为回馈长期客户，我们设立了会员等级。**新会员**（首次订阅）无额外折扣；**银牌会员**（订阅满3个月），可享受**核心餐食计划费用**的 5% 折扣；**金牌会员**（订阅满12个月），可享受**每周总费用**（包含所有项目及配送费）的 10% 折扣。

### 输入描述：

一段包含客户的会员状态（新会员/订阅时长）、餐食计划、食谱偏好、加购选项以及配送区域的自然语言描述。

### 输出描述：

```json
{
  "weekly_total_bill": int, //最终每周需支付的总金额
  "cost_breakdown": {
        "base_plan_cost": int, //折扣后的核心餐食计划费用
        "preference_surcharge": int, //食谱偏好附加费
        "add_ons_cost": int,  //精选加购的总费用
        "delivery_cost": int  //配送区域费用
  },
  "applied_rules": ["\[M.X\]"] //触发的规则编号，按编号升序排列
}
```
